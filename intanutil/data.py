...
def read_one_block(header, fid):
    """Reads one 60 or 128 sample data block from fid into data,
    at the location indicated by indices."""
    samples_per_block = header['num_samples_per_data_block']
    format_sign = 'i' if get_timestamp_signed(header) else 'I'
    format_expression = '<' + format_sign * samples_per_block
    read_length = 4 * samples_per_block

    # print('stamp: ' + str(fid.tell()))
    timestamp = np.array(struct.unpack(format_expression, fid.read(read_length)))
    # print('timestamp: ' + str(fid.tell()))
    amplifier_data = analog_read(fid, samples_per_block, header['num_amplifier_channels'])
    # print('amplifier_data: ' + str(fid.tell()))
    aux_input_data = analog_read(fid, int(samples_per_block / 4), header['num_aux_input_channels'])
    # print('aux_input_data: ' + str(fid.tell()))
    supply_voltage_data = analog_read(fid, 1, header['num_supply_voltage_channels'])
    # print('supply_voltage_data: ' + str(fid.tell()))
    temp_sensor_data = analog_read(fid, 1, header['num_temp_sensor_channels'])
    # print('temp_sensor_data: ' + str(fid.tell()))
    board_adc_data = analog_read(fid, samples_per_block, header['num_board_adc_channels'])
    # print('board_adc_data: ' + str(fid.tell()))
    digital_in = np.array(struct.unpack('<' + 'H' * samples_per_block, fid.read(2 * samples_per_block)))
    # print('digital_in: ' + str(fid.tell()))
    digital_out = np.array(struct.unpack('<' + 'H' * samples_per_block, fid.read(2 * samples_per_block)))
    # print('digital_out: ' + str(fid.tell()))

    # print(read_length, samples_per_block * header['num_amplifier_channels'], int(samples_per_block / 4) * header['num_aux_input_channels'],
    #       header['num_supply_voltage_channels'], header['num_temp_sensor_channels'], samples_per_block * header['num_board_adc_channels'],
    #       2 * samples_per_block, 2 * samples_per_block)

    return timestamp, amplifier_data, aux_input_data, supply_voltage_data, temp_sensor_data, board_adc_data, digital_in, digital_out
